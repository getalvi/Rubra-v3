  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium text-[#9aa0a6] hover:text-[#e8eaed] hover:bg-[#2a2a2e] transition-all duration-150"
        aria-expanded={open}
      ...
      {open && (
        <div
          className="absolute right-0 top-full mt-2 w-52 sm:w-56 rounded-xl bg-[#2a2a2e] border border-[#3c4043] shadow-lg z-50 overflow-hidden"
          role="listbox"
        >
          {modes.map(m => {
            const Icon = m.icon;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className={`text-xs sm:text-sm font-medium ${isActive ? 'text-[#8ab4f8]' : 'text-[#e8eaed]'}`}>
                    {m.label}
                  </div>
                  <div className="text-[10px] sm:text-xs text-[#9aa0a6] truncate">{m.desc}</div>
                </div>
