/**
 * MessageThread — Gemini-style two-sided chat
 * User messages: RIGHT side (bubble)
 * AI messages: LEFT side (flat, with avatar)
 * Thinking: animated dots inline (no top/bottom bar)
 */
...
// ── Gemini-style animated thinking dots ───────────────────────────────────────
      ...
      <div className="flex items-center gap-[5px]">
        {[0, 1, 2].map(i => (
          <span
            ...
            style={{ animation: `thinkBounce 1.2s ${i * 0.2}s ease-in-out infinite` }}
          />
        ))}
      </div>
      {label && (
        <span className="text-[12.5px] text-white/40 font-medium">{label}</span>
      )}
    </div>
  )
}
...
function AIActions({ content }) {
  return (
    <div className="flex items-center gap-0.5 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <button className="p-1.5 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/[0.06] transition-all" title="Good response">
        <ThumbsUp size={14} />
      </button>
      <button className="p-1.5 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/[0.06] transition-all" title="Bad response">
        <ThumbsDown size={14} />
      </button>
  ...
  return (
    <div className="group flex items-start gap-3 mb-4 px-4">
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        {/* Thinking state */}
        {isThinking && (
          <ThinkingDots label={message.statusHint || ''} />
        )}
...
// ── MessageThread ─────────────────────────────────────────────────────────────
export default function MessageThread({ messages, statusText }) {
  ...
  // Inject statusHint into the currently-streaming bot message
  const enriched = messages.map(m =>
    m.streaming && m.role === 'assistant'
      ? { ...m, statusHint: statusText }
      : m
  )
