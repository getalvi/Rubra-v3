 ...
 * Fixed:
 *   - statusText + progress passed to ChatBar
 *   - WelcomeScreen suggestion click: onPrompt fires send() directly (no async race)
 *   - onSend wrapped to prevent double-fire
 */
...
function WelcomeScreen({ onPrompt, isStreaming }) {
  ...
  const handleClick = useCallback((prompt) => {
    // Debounce: prevent double-fire from fast clicks
    const now = Date.now()
        ...
        {SUGGESTIONS.map(({ Icon, label, prompt, color, bg, border }) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11.5px] text-white/32 leading-snug mt-0.5 line-clamp-2">
                {prompt.slice(0, 48)}…
              </p>
            </div>
          </button>
  ...
  return (
    <div className="flex items-center justify-between px-4 h-[52px] border-b border-white/[0.05] shrink-0 backdrop-blur-glass bg-white/[0.012]">
      ...
      <div className="flex items-center gap-2">
        {/* Live status in header while streaming */}
        {isStreaming && statusText && (
          <span className="text-[11.5px] text-white/35 animate-pulse hidden sm:block">
            {statusText}
          </span>
        )}
        ...
        {agent ? (
          <div className={clsx(
            'flex items-center gap-2 px-3 py-1 rounded-full glass-card border animate-fade-up',
            agent.ring
          )}>
