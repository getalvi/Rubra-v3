/**
 * ChatWindow — Main chat layout
 * Fixed: correct imports, all props used properly
 */
...
function WelcomeScreen({ onPrompt, isStreaming }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-5 pb-8 animate-fade-in overflow-y-auto">
...
// ── Top bar ───────────────────────────────────────────────────────────────────
function TopBar({ onMenuToggle, onNewChat, agentMeta, isStreaming, statusText }) {
  const agent = agentMeta ? (AGENT_COLORS[agentMeta.agent] || null) : null

  return (
    <div className="flex items-center justify-between px-4 h-[52px] border-b border-white/[0.06] shrink-0 bg-[#131314]/80 backdrop-blur-sm">
      <button
      ...
      <div className="flex items-center gap-2">
        {isStreaming && statusText && (
          <span className="text-[11px] text-white/35 animate-pulse hidden sm:block">
            {statusText}
          </span>
        )}
        ...
        {agent ? (
          <div className={clsx(
            'flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.04] border animate-fade-in',
            agent.ring
          )}>
            <span className={clsx('w-[6px] h-[6px] rounded-full animate-pulse', agent.dot)} />
            <span className="text-[12px] text-white/60 font-medium">{agent.label}</span>
          </div>
        ) : (
