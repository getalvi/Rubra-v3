...
const SUGGESTIONS = [
  ...
  { Icon: Brain,    label: 'Explain',       prompt: 'Explain transformer attention mechanism step by step',                      color: 'text-violet-400/80',  bg: 'bg-violet-500/[0.09]',  border: 'border-violet-500/[0.12]' },
  ...
  { Icon: Globe,    label: 'Browse web',    prompt: 'Browse openai.com and summarize the latest blog posts',                     color: 'text-blue-400/80',    bg: 'bg-blue-500/[0.09]',    border: 'border-blue-500/[0.12]' },
...
function WelcomeScreen({ onPrompt, isStreaming }) {
  ...
  const handle = useCallback((p) => {
    const now = Date.now()
    if (now - lastClick.current < 500 || isStreaming) return
    lastClick.current = now; onPrompt(p)
  }, [onPrompt, isStreaming])

  return (
    <div className="flex-1 flex flex-col items-center justify-center px-5 pb-8 overflow-y-auto">
      <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl mb-5 shadow-lg">R</div>
      <h1 className="text-[22px] font-semibold text-white/88 mb-2 tracking-tight">How can I help you?</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 w-full max-w-[620px]">
        {SUGGESTIONS.map(({ Icon, label, prompt, color, bg, border }) => (
          <button key={label} onClick={() => handle(prompt)} disabled={isStreaming}
            className={clsx('flex items-start gap-3 p-4 rounded-2xl text-left border transition-all duration-200',
              bg, border, 'hover:brightness-110 hover:-translate-y-0.5 active:scale-[0.97]',
              isStreaming && 'opacity-40 cursor-not-allowed')}>
            <Icon size={16} className={clsx('shrink-0 mt-0.5', color)} />
            <div>
              <p className="text-[12.5px] font-medium text-white/75">{label}</p>
              <p className="text-[11px] text-white/32 leading-snug mt-0.5 line-clamp-2">{prompt.slice(0, 48)}…</p>
            </div>
          </button>
        ))}
      </div>
...
function TopBar({ onMenuToggle, onNewChat }) {
  return (
    <div className="flex items-center justify-between px-4 h-[52px] border-b border-white/[0.06] shrink-0">
      <button onClick={onMenuToggle} className="w-8 h-8 flex items-center justify-center rounded-lg text-white/30 hover:text-white/65 hover:bg-white/[0.06] transition-all"><Menu size={17} /></button>
      ...
      <button onClick={onNewChat} className="w-8 h-8 flex items-center justify-center rounded-lg text-white/30 hover:text-white/65 hover:bg-white/[0.06] transition-all" title="New chat"><Plus size={17} /></button>
    </div>
  )
}

export default function ChatWindow({ messages, isStreaming, onSend, onFile, onStop, onNewChat, onMenuToggle }) {
  ...
  return (
    <div className="flex flex-col h-full min-w-0 flex-1">
      ...
      <ChatBar onSend={onSend} onFile={onFile} onStop={onStop} isStreaming={isStreaming} />
    </div>
  )
}
