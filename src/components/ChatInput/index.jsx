...
const MODES = [
  {
    id: "fast",
    label: "Rubra Fast",
    desc: "Quick answers, no deep thinking",
    dot: "#22c55e",
    mode: "fast",
  },
  {
    id: "think",
    label: "Rubra Think",
    desc: "Deeper reasoning, slower response",
    dot: "#60a5fa",
    mode: "hermes",
  },
  {
    id: "agent",
    label: "Rubra Agent",
    desc: "Multi-step agentic task execution",
    dot: "#a78bfa",
    mode: "agent",
  },
];
  ...
  return (
    <div className="relative" onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget)) setOpen(false); }}>
      <button
        ...
        className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs transition-colors select-none"
        style={{
          color: disabled ? "#3a3a55" : "#8080a0",
          border: "1px solid #1e1e2e",
          background: open ? "#1a1a2a" : "transparent",
          cursor: disabled ? "default" : "pointer",
        }}
        onMouseEnter={e => { if (!disabled) e.currentTarget.style.background = "#1a1a2a"; }}
        onMouseLeave={e => { if (!open) e.currentTarget.style.background = "transparent"; }}
      >
        <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: disabled ? "#3a3a55" : cur.dot }}/>
        <span className="font-medium">{cur.label}</span>
        <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round"
          style={{ opacity: 0.5, transform: open ? "rotate(180deg)" : "rotate(0deg)", transition:"transform .15s" }}>
          <path d="M6 9l6 6 6-6"/>
        </svg>
      </button>

      {open && (
        <div className="absolute bottom-full mb-1.5 left-0 z-50 rounded-xl overflow-hidden shadow-2xl min-w-[200px]"
          ...
          {MODES.map(m => (
            <button
              ...
              style={{ background: selected === m.id ? "#1a1a2a" : "transparent" }}
              onMouseEnter={e => e.currentTarget.style.background = "#161622"}
              onMouseLeave={e => e.currentTarget.style.background = selected === m.id ? "#1a1a2a" : "transparent"}
            >
              <span className="w-2 h-2 rounded-full flex-shrink-0 mt-1" style={{ background: m.dot }}/>
              <div>
                <div className="text-xs font-medium" style={{ color: selected === m.id ? "#ffffff" : "#c0c0d8" }}>
                  {m.label}
                  {selected === m.id && <span className="ml-2 text-[10px]" style={{ color:"#e8301f" }}>●</span>}
                </div>
                <div className="text-[11px] mt-0.5" style={{ color:"#5a5a7a" }}>{m.desc}</div>
              </div>
            </button>
          ))}
        </div>
...
export default function ChatInput({ onSend, onStop, isStreaming, disabled }) {
  ...
  const [modeId, setModeId] = useState("fast"); // "fast" | "think" | "agent"
  ...
  const submit = () => {
    ...
    for (const a of atts) {
      ...
      if (r.type === "image") {
        ...
        full = full ? `${full}\n\n[File: ${r.filename}]` : `[File: ${r.filename}]`;
      }
    }
    ...
    onSend(full, getMode()); // ← pass mode as second arg
  ...
  return (
    <div className="px-4 pb-5 max-w-3xl w-full mx-auto"
      ...
      <div className="rounded-xl overflow-hidden"
        ...
        {/* ── Bottom bar: mode selector + attach + streaming bars ── */}
